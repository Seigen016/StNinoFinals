...
"[project]/app/api/students/update-rfid/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        if (updateData.email) {
            // Find by email
            const { data, error } = await admin.from('users').select('*').eq('role', 'student').eq('email', updateData.email.toLowerCase().trim()).limit(10);
            students = data || [];
            findError = error;
        } else if (updateData.studentId) {
            // Find by student ID - get all and filter in memory to avoid column errors
            const { data: allStudents, error } = await admin.from('users').select('*').eq('role', 'student').limit(1000);
        ...
        const { data: updatedStudent, error: updateError } = await admin.from('users').update(updateFields).eq('id', student.id).select().single();
        if (updateError) {
            console.error('Database error updating RFID:', updateError);
            console.error('Error message:', updateError.message);
            // Check if error is about missing column
            if (updateError.message && (updateError.message.includes('column') && updateError.message.includes('does not exist') || updateError.message.includes('rfid'))) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$0$2e$8_react$2d$dom$40$19$2e$2$2e$1_react$40$19$2e$2$2e$1_$5f$react$40$19$2e$2$2e$1$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    success: false,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__610b54fd._.js.map